...
} from "@/components/modals/imageModal";
				...
				<div className="max-w-4xl mx-auto">
					<div className="bg-muted-background rounded-lg p-8 md:p-10 space-y-6">
						...
						<p className="text-sm text-muted-foreground">
							{t("hero.currently_learning")}{" "}
							<span className="text-accent">TanStack Start</span>
						</p>
					</div>
				</div>
			</section>
					...
					<div className="space-y-4">
						{projects.map((project) => (
							<ProjectCard
								key={project.name}
								project={project}
								onOpenImages={(images) => setImageModal({ images, index: 0 })}
							/>
						))}
					</div>
				</div>
